...
VPN_STATUS="$($VPN_GET_STATUS | cut -d' ' -f1)"	# returns Connected/Connecting/<other>
CONNECTED=Connected
...
vpn_report() {
  while true;
  do 
      mullvad status | cut -d' ' -f1,3 && sleep 10s
  done
}
...
rofi_theme="default"
rofi_location="-location 2 -xoffset 0 -yoffset 0"
